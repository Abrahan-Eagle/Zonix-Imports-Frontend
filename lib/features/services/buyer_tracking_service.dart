...
class BuyerTrackingService {
  ...
  Future<Map<String, dynamic>> getOrderStatus(int orderId) async {
    try {
      ...
      final url = Uri.parse('${EnvHelper.apiUrl}/api/buyer/tracking/order/$orderId');
  ...
  Future<Map<String, dynamic>> getDeliveryAgentLocation(int orderId) async {
    try {
      final headers = await AuthHelper.getAuthHeaders();
      final url = Uri.parse('${EnvHelper.apiUrl}/api/buyer/tracking/delivery-agent/$orderId');
  ...
  Future<bool> updateOrderStatus({
    ...
    try {
      final headers = await AuthHelper.getAuthHeaders();
      final url = Uri.parse('${EnvHelper.apiUrl}/api/buyer/tracking/order/$orderId/status');
