...
import 'package:zonix/features/profile/data/models/phones/phone.dart';
...
class PhoneService {
  ...
  Future<List<Phone>> fetchPhones(int id) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        logger.e(
            'Error fetching phones: ${response.statusCode} - ${response.body}');
  ...
  Future<Phone> createPhone(Phone phone, int userId) async {
    try {
      ...
      if (response.statusCode == 201) {
        ...
        logger.e(
            'Error creating phone: ${response.statusCode} - ${response.body}');
  ...
  Future<Phone> updatePhone(int id, Map<String, dynamic> updates) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        logger.e(
            'Error updating phone: ${response.statusCode} - ${response.body}');
  ...
  Future<void> updatePrimaryStatus(int id, bool isPrimary) async {
    ...
    if (isPrimary) {
      try {
        final phones =
            await fetchPhones(56); // Obtener todos los teléfonos del usuario
  ...
  Future<void> deletePhone(int id) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        logger.e(
            'Error deleting phone: ${response.statusCode} - ${response.body}');
  ...
  Future<List<Map<String, dynamic>>> fetchOperatorCodes() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        logger.e(
            'Error fetching operator codes: ${response.statusCode} - ${response.body}');
        throw Exception(
            'Error al obtener códigos de operador: ${response.statusCode}');
      }
    } catch (e) {
  ...
  Future<void> updateStatusCheckScanner(int userId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        logger.e(
            'Error updating status check scanner: ${response.statusCode} - ${response.body}');
        throw Exception(
