...
import 'package:zonix/features/profile/data/models/addresses/adresse.dart';
import 'package:zonix/features/profile/data/models/addresses/models.dart';
...
class AddressService {
  ...
  Future<List<StateModel>> fetchStates(int countryId) async {
    logger.e(
        '1111111111111111111111111111111111111111111111111111111111111111111111111: $countryId');
    final token = await _getToken();
    try {
  ...
  Future<List<City>> fetchCitiesByState(int stateId) async {
    logger.e(
        '2222222222222222222222222222222222222222222222222222222222222222222222222: $stateId');
    final token = await _getToken();
    try {
      final response = await http
          .post(
  ...
  Future<void> createAddress(Address address, int userId) async {
    logger.w(
        'addressUSERID: ${address.id}  userIduserIduserIdUSERID: $userId  createAddress: ${address.latitude} ${address.longitude}');
    final token = await _getToken();
    try {
      final response = await http
          .post(
  ...
  _handleResponse<T>(http.Response response, T Function(dynamic) fromJson) {
    if (response.statusCode == 200) {
      ...
      logger
          .e('Error en la solicitud: ${response.statusCode} ${response.body}');
      throw ApiException(
